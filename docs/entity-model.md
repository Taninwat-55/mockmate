...
erDiagram
    ...
    FEEDBACK {
        string id PK
        string sessionId FK
        int roleKnowledgeScore
        string roleKnowledgeStrength
        string roleKnowledgeWeakness
        string roleKnowledgeTip
...
| jobDescription | String? | Optional (#44), max 6,000 chars | Job posting pasted by the user |
| resume | String? | Optional (#44), max 6,000 chars | Resume text pasted by the user |
| title | String | Required, max 120 chars | The role the user is interviewing for (e.g. "Barista at Espresso House") — also the session label |
| seniority | Enum | Default: JUNIOR | STUDENT / ENTRY / JUNIOR / MID_SENIOR — calibrates questions and grading (#44) |
| workSetting | Enum? | Optional | ONSITE / HYBRID / REMOTE (#44) |
| employmentType | Enum? | Optional | FULL_TIME / PART_TIME / INTERNSHIP (#44) |
...
| roleKnowledgeScore | Int | Required, 1–5 | Score for the role knowledge dimension (was roleKnowledge*, renamed in #44) |
| roleKnowledgeStrength | String | Required | One written strength observation |
| roleKnowledgeWeakness | String | Required | One written weakness observation |
| roleKnowledgeTip | String | Required | One actionable improvement tip |
