...
flowchart TD
    ...
    NextApp -->|"after() — fire-and-forget\npaid session COMPLETED"| EmailSvc
...
| **Resend** | Sends the post-session summary email for paid sessions — called from the feedback route via `after()`, so it never blocks the feedback page (#54; replaced the AWS Lambda) | Resend |
...
Browser triggers end (5 questions complete or user ends early) → Next.js API route calls Gemini with all evaluation notes to generate grading matrix JSON → Feedback row created in Neon → Session status updated to COMPLETED → for paid sessions, Next.js schedules the summary email with `after()` (sent via Resend once the response is out) → Next.js returns grading data to browser → Feedback page renders
...
All AWS resources (S3, CloudFront, IAM roles and policies — Phase 2) are provisioned using Terraform configuration files stored in `/infra/`. This means:
...
Terraform manages: S3 bucket with access policies, CloudFront distribution pointed at S3, and least-privilege IAM roles.
...
|---|---|
| S3 + CloudFront | Resume PDF upload replaces plain text textarea. Files stored in S3, served via CloudFront. |
